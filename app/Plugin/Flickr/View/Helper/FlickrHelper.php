...
class FlickrHelper extends AppHelper {
 ...
 protected $_captionDefaults = array(
  ...
  /**
   * add mizoshiri
   * For Flickr simple gallery with Lightview(jQuery photo library) 
   * 
   * @param type $data
   * @param type $formatAttribs optional Special key: type (see example).
   * @param type $linkAttribs optional Attributes for the <a> wrapping the thumbnail
   * @param type $thumbAttribs optional Attributes for the <img> containing the thumbnail
   * @param type $imgAttribs optional Size for the large image. Default: 'size' => 'n'
   * @return type 
   */
  
  public function getPhotoLists($data,
  $formatAttribs = array(),
  $linkAttribs = array(),
  $thumbAttribs = array(),
  $imgAttribs = array()
  ...
  foreach ($data as $p) {
    if($p!==null){
      ...
      $linkAttribs = array_merge($linkAttribs,array('data-lightview-title' => $p['title'], 'data-lightview-group' => 'flickr' , 'data-lightview-caption' => $p['description']['_content']));
